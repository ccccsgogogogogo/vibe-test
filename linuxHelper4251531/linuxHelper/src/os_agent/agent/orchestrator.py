...
from os_agent.agent.action_schema import (
...
class Orchestrator:
    ...
    def _generate_generic_shell_command(self, user_text: str, profile: str, context_hint: str = "") -> PlannedCommand:
        action_plan = self._generate_action_plan_with_model(user_text, profile, context_hint=context_hint)

        if action_plan is None or not action_plan.is_available:
            abort_reason = action_plan.abort_reason if action_plan else ""
            get_logger().warning(
                "LLM action plan unavailable for request: %s, reason: %s",
                user_text[:120],
                abort_reason or "parse_failure",
            )
        ...
        command = action_plan.to_shell_command()
        get_logger().info(
            "LLM action plan: plan_type=%s steps=%d explanation=%s",
            action_plan.plan_type,
            len(action_plan.commands),
            action_plan.explanation,
        )
        return PlannedCommand(intent="generic_shell", command=command)

    def _generate_action_plan_with_model(
        self, user_text: str, profile: str, context_hint: str = ""
    ) -> ActionPlan | None:
        prompt = [
            {
                "role": "system",
                "content": BUILTIN_ACTION_PLAN_PROMPT,
            },
            {
                "role": "user",
                "content": (
                    f"User request: {user_text}\n"
                    f"Target Linux profile: {profile}\n"
        ...
        get_logger().debug("LLM action plan raw output: %s", generated[:500])

        return parse_action_plan(generated)
