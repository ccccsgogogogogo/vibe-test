...
class Orchestrator:
    ...
    def handle_turn(
        ...
        try:
            ...
            if clarification_action == "none" and self._is_ambiguous_request(raw_user_text):
                guessed_request = self._guess_request_from_memory(raw_user_text, self.turn_memory)
                if guessed_request:
                    self.pending_intent_guess = {
                        "original_request": raw_user_text,
    ...
    def _simple_result_summary(result: LinuxCommandResult) -> str:
        ...
        short = (result.stderr or "命令执行失败。")[:120].strip()
        return f"执行失败，返回码 {result.return_code}。{short}"
    ...
    def _is_composite_request(user_text: str) -> bool:
        ...
        connectors = ["并", "然后", "再", "后", "之后", "同时", "且", "and", "then"]
        action_keywords = [
