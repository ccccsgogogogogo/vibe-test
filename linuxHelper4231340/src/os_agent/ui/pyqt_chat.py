...
from os_agent.config import AppConfig, SSHConfig, load_config
from os_agent.logging_config import get_logger, log_connection
from os_agent.models import build_model_client
from os_agent.scheduler import TaskScheduler, ScheduledTask, ScheduleType
...
class ScheduleDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        type_label = QLabel("定时类型:")
        self.type_combo = QComboBox()
        self.type_combo.addItem("单次执行", ScheduleType.ONCE)
        self.type_combo.addItem("每天执行", ScheduleType.DAILY)
        self.type_combo.addItem("间隔执行", ScheduleType.INTERVAL)
        ...
        self.time_input.setPlaceholderText("HH:MM 或 间隔秒数")
        time_layout.addWidget(time_label)
        time_layout.addWidget(self.time_input, 1)
        
        # 任务命令
        command_label = QLabel("任务命令:")
        self.command_input = QPlainTextEdit()
        self.command_input.setPlaceholderText("输入要定时执行的命令...")
        self.command_input.setFixedHeight(120)
        
        # 对话绑定提示
        session_info_label = QLabel("此定时任务将绑定到当前对话\n定时任务只在绑定的对话中执行，即使您切换到其他对话")
    ...
    def _on_type_changed(self) -> None:
        ...
        if schedule_type == ScheduleType.ONCE:
            self.time_input.setPlaceholderText("HH:MM (如: 14:30)")
        elif schedule_type == ScheduleType.DAILY:
            self.time_input.setPlaceholderText("HH:MM (如: 09:00)")
        elif schedule_type == ScheduleType.INTERVAL:
            self.time_input.setPlaceholderText("间隔秒数 (如: 3600)")
    ...
    def _on_ok(self) -> None:
        ...
        if not time_str:
            QMessageBox.warning(self, "输入错误", "请输入执行时间")
            return
        
        if not command:
            QMessageBox.warning(self, "输入错误", "请输入任务命令")
            return
        
        # 验证时间格式
        schedule_type = self.type_combo.currentData()
        if schedule_type in [ScheduleType.ONCE, ScheduleType.DAILY]:
            if not re.match(r'^\d{1,2}:\d{2}$', time_str):
                QMessageBox.warning(self, "输入错误", "时间格式应为 HH:MM")
                return
            hour, minute = map(int, time_str.split(':'))
            if hour < 0 or hour > 23 or minute < 0 or minute > 59:
                QMessageBox.warning(self, "输入错误", "时间值无效")
                return
        elif schedule_type == ScheduleType.INTERVAL:
            if not time_str.isdigit() or int(time_str) <= 0:
                QMessageBox.warning(self, "输入错误", "间隔时间应为正整数")
                return
...
class ServerLoginDialog(QDialog):
    ...
    def __init__(self, cfg: AppConfig, parent: QWidget | None = None) -> None:
        ...
        title = QLabel("请输入 Linux 服务器连接信息")
        ...
        self.host_input.setText((cfg.ssh.host if cfg.ssh else "").strip())

        self.port_input = QLineEdit()
        self.port_input.setPlaceholderText("22")
        self.port_input.setText(str(cfg.ssh.port if cfg.ssh else 22))

        self.username_input = QLineEdit()
        self.username_input.setPlaceholderText("root")
        self.username_input.setText((cfg.ssh.username if cfg.ssh else "root").strip())

        self.password_input = QLineEdit()
        self.password_input.setEchoMode(QLineEdit.EchoMode.Password)
        self.password_input.setPlaceholderText("请输入密码")
        self.password_input.setText((cfg.ssh.password if cfg.ssh and cfg.ssh.password else "").strip())
    ...
    def _on_submit(self) -> None:
        if self.build_ssh_config() is None:
            return
        self.accept()
    ...
    def build_ssh_config(self) -> SSHConfig | None:
        ...
        if not host:
            QMessageBox.warning(self, "输入错误", "请填写服务器 IP/主机地址。")
            return None
...
class ChatWindow(QMainWindow):
    ...
    # 自定义信号：用于从后台线程安全地触发主线程中的定时任务执行
    scheduled_task_signal = pyqtSignal(str, str)  # command, session_id
    ...
    def __init__(self, cfg: AppConfig | None = None) -> None:
        ...
        # 初始化核心组件
        self.cfg = cfg if cfg is not None else load_config()  # 加载配置文件
        ...
        self.scheduler.set_callback(self._on_scheduled_task_execute)
        self.scheduler.start()
        
        # 连接定时任务信号到主线程执行方法（线程安全）
        self.scheduled_task_signal.connect(self._execute_scheduled_command)
        
        # 当前对话的定时任务列表
        self.current_session_tasks: list[ScheduledTask] = []
        self.current_session_id: str = "session_1"  # 默认对话ID

        # 设置对话保存目录（按服务器IP拆分文件）
        self._conversations_dir = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'conversations')
    ...
    def _load_conversations(self) -> None:
        """
        Load conversation history from per-server JSON file.

        Attempts to read the JSON file corresponding to the current server IP
        and restore all previous conversations with their message histories.
        If the file doesn't exist or is corrupted, falls back to default
        conversation setup.
        """
    ...
    def _save_conversations(self) -> None:
        """
        Save all conversations to per-server JSON file.

        Serializes the current conversation list and their message histories
        to a JSON file specific to the current server IP for persistence
    ...
    def _get_conversations_file(self) -> str:
        host = "local"
        if self.cfg.ssh_enabled and self.cfg.ssh and self.cfg.ssh.host:
            host = self.cfg.ssh.host.strip()
        return os.path.join(self._conversations_dir, f"{host}.json")

    def _load_brand_icon(self) -> None:
        icon_path = Path(__file__).resolve().parent.parent.parent.parent / "icons" / "icon.png"
        if icon_path.exists():
    ...
    def _build_sidebar(self, parent_layout: QHBoxLayout) -> None:
        ...
        brand_container.addWidget(self.brand_icon)
    ...
    def _build_main_panel(self, parent_layout: QHBoxLayout) -> None:
        ...
        self.chat_scroll.verticalScrollBar().rangeChanged.connect(self._on_chat_scroll_range_changed)
        ...
        parent_layout.addWidget(self.schedule_panel)  # 定时任务面板

    def _build_schedule_panel(self) -> QWidget:
        """构建右侧定时任务面板。"""
        # 创建定时任务面板
        panel = QFrame()
        panel.setObjectName("schedulePanel")
        ...
        """)
        add_button.clicked.connect(self._on_add_schedule)
    ...
    def _refresh_schedule_list(self) -> None:
        ...
        # 过滤出属于当前对话的任务
        current_session_tasks = [task for task in tasks if task.session_id == self.current_session_id]
        self.current_session_tasks = current_session_tasks
        
        if not current_session_tasks:
            return
        
        for task in current_session_tasks:
            item = QListWidgetItem()
            widget = self._create_schedule_item_widget(task)
            item.setSizeHint(widget.sizeHint())
            self.schedule_list.addItem(item)
            self.schedule_list.setItemWidget(item, widget)
    
    def _create_schedule_item_widget(self, task: ScheduledTask) -> QWidget:
        """创建定时任务列表项的小部件。"""
        widget = QWidget()
        ...
        """)
        edit_button.clicked.connect(lambda: self._on_edit_schedule(task.id))
        ...
        """)
        delete_button.clicked.connect(lambda: self._on_delete_schedule(task.id))
        ...
        # 任务命令（截断显示）
        command_text = task.command[:50] + "..." if len(task.command) > 50 else task.command
        command_label = QLabel(f"命令: {command_text}")
        ...
        # 定时信息
        schedule_info = f"{task.schedule_type.value} {task.scheduled_time}"
        if task.next_execution:
            from datetime import datetime
            next_time = datetime.fromisoformat(task.next_execution).strftime("%m-%d %H:%M")
            schedule_info += f" (下次: {next_time})"
        ...
        # 绑定对话信息
        session_info = f"绑定到: {task.session_id}"
        if task.session_id == self.current_session_id:
            session_info += " (当前对话)"
        ...
        bottom_layout.addWidget(session_label)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if task_data:
                task = ScheduledTask(
                    ...
                    session_id=self.current_session_id,  # 绑定到当前对话
                    created_at=datetime.now().isoformat()
                )
                
                if self.scheduler.add_task(task):
                    self._refresh_schedule_list()
                    get_logger().info(f"定时任务创建成功: {task.name}，绑定到对话 {self.current_session_id}")
    
    def _on_scheduled_task_execute(self, command: str, session_id: str) -> None:
        """定时任务执行回调。
        
        使用 Qt 信号机制将执行请求从后台线程安全地传递到主线程。
        信号槽是线程安全的，Qt 会自动处理跨线程调用。
        """
        get_logger().info(f"[回调触发] 定时任务回调被调用: command='{command}', session_id={session_id}")
        
        try:
            # 发射信号，Qt 会自动在主线程中执行槽函数
            self.scheduled_task_signal.emit(command, session_id)
            get_logger().info(f"[信号发射] 已发射定时任务信号到主线程: {command}")
        except Exception as e:
            get_logger().error(f"[回调错误] 定时任务信号发射失败: {e}", exc_info=True)
    
    def _execute_scheduled_command(self, command: str, session_id: str) -> None:
        """执行定时任务命令。定时任务只在绑定的对话中执行。"""
        get_logger().info(f"执行定时任务: {command} (绑定到对话: {session_id})")
        
        # 首先，在当前对话中显示系统通知，让用户知道定时任务正在执行
        current_time = datetime.now().strftime("%H:%M:%S")
        if self.current_session_id != session_id:
            # 如果用户不在绑定对话中，显示一个系统通知
            notification = f"定时任务正在执行: '{command}' (绑定到对话: {session_id}, 时间: {current_time})"
            self._append_to_session("System", notification, self.current_session_id)
        
        # 保存当前对话状态
        original_session_id = self.current_session_id
        original_session_index = self.session_list.currentRow()
        
        try:
            # 切换到定时任务绑定的对话
            if not self._switch_to_session_for_task(session_id):
                get_logger().warning(f"无法切换到对话 {session_id}，定时任务将在后台执行")
                # 即使无法切换，也记录任务执行（在后台）
                self._record_scheduled_task_execution(session_id, command)
                return
            
            # 在绑定对话中执行任务
            get_logger().info(f"在对话 {session_id} 中执行定时任务")
            
            # 在绑定对话中显示用户消息
            self._append_to_session("User", command, session_id)
            
            # 执行命令（在绑定对话的上下文中）
            QTimer.singleShot(100, lambda: self._process_scheduled_command_in_session(command, session_id))
            
        finally:
            # 恢复原始对话（如果用户没有手动切换）
            if self.current_session_id == session_id and original_session_id != session_id:
                self._restore_original_session(original_session_index, original_session_id)
    
    def _switch_to_session_for_task(self, session_id: str) -> bool:
        """为定时任务切换到指定的对话。
        
        与普通切换不同，这个方法不会触发UI更新，只改变内部状态。
        
        Args:
            session_id: 对话ID，格式为 "session_X"
            
        Returns:
            bool: 是否成功切换到指定对话
        """
        try:
            # 从session_id中提取索引（session_1 -> 0, session_2 -> 1, ...）
            if session_id.startswith("session_"):
                session_index = int(session_id.split("_")[1]) - 1
                
                # 检查索引是否有效
                if 0 <= session_index < self.session_list.count():
                    # 更新内部状态，但不触发UI事件
                    self.session_list.blockSignals(True)
                    self.session_list.setCurrentRow(session_index)
            ...
            get_logger().warning(f"为定时任务切换对话失败: {session_id}, 错误: {e}")
        
        return False
    
    def _append_to_session(self, role: str, text: str, session_id: str) -> None:
        """向指定对话添加消息。
        ...
        try:
            ...
            if session_id.startswith("session_"):
                ...
                # 确保历史记录列表足够长
                while len(self.session_histories) <= session_index:
                    self.session_histories.append([])
                
                # 添加到指定会话的历史记录
                message_payload = {"role": role, "text": text}
                self.session_histories[session_index].append(message_payload)
                
                # 如果当前正在查看这个对话，则更新UI
                if self.current_session_id == session_id:
                    self._append_message_widget(role, text)
                    self._request_scroll_chat_to_bottom()
                
                # 保存对话列表
                self._save_conversations()
                
                get_logger().debug(f"向对话 {session_id} 添加{role}消息")
        except (ValueError, IndexError) as e:
            get_logger().error(f"向对话添加消息失败: {session_id}, 错误: {e}")
    
    def _process_scheduled_command_in_session(self, command: str, session_id: str) -> None:
        """在指定对话的上下文中处理定时任务命令。
        ...
        try:
            ...
            if self.current_session_id != session_id:
                if not self._switch_to_session_for_task(session_id):
                    get_logger().error(f"无法在对话 {session_id} 中处理命令")
                    return
            
            # 处理命令（模拟用户发送）
            self._start_turn_processing(command, confirmed=False)
            
            get_logger().info(f"定时任务在对话 {session_id} 中处理完成: {command}")
            
        except Exception as e:
            get_logger().error(f"定时任务处理失败: {e}")
            # 在绑定对话中记录错误
            self._append_to_session("System", f"定时任务执行失败: {str(e)}", session_id)
    
    def _record_scheduled_task_execution(self, session_id: str, command: str) -> None:
        """记录定时任务执行（用于无法切换对话的情况）。
        ...
        try:
            # 在绑定对话的历史记录中添加执行记录
            self._append_to_session("System", 
                f"定时任务已执行: {command} (执行时间: {datetime.now().strftime('%H:%M:%S')})", 
                session_id)
            
            get_logger().info(f"记录定时任务执行到对话 {session_id}: {command}")
        except Exception as e:
            get_logger().error(f"记录定时任务执行失败: {e}")
    
    def _restore_original_session(self, original_index: int, original_session_id: str) -> None:
        """恢复原始对话。
        
        Args:
            original_index: 原始对话索引
            original_session_id: 原始对话ID
        """
        try:
            # 检查用户是否已经手动切换了对话
            if self.current_session_id != original_session_id:
                # 恢复原始对话
                self.session_list.blockSignals(True)
                self.session_list.setCurrentRow(original_index)
                self.session_list.blockSignals(False)
                ...
                get_logger().debug(f"恢复原始对话: {original_session_id}")
        except Exception as e:
            get_logger().warning(f"恢复原始对话失败: {e}")
    
    def _send_message_directly(self, command: str) -> None:
        """直接发送消息，绕过所有检查。"""
        try:
            # 清除输入框
            self.input.clear()
            ...
            get_logger().info(f"定时任务执行完成: {command}")
            
        except Exception as e:
            get_logger().error(f"定时任务发送失败: {e}")
            self._append("System", f"定时任务发送失败: {str(e)}")
    
    def _ensure_healthy_connection(self) -> None:
        """确保连接健康，避免定时任务使用失效连接。"""
        try:
            # 强制清理现有连接，确保定时任务使用新连接
            if hasattr(self.orchestrator.executor, '_cleanup_connection'):
                self.orchestrator.executor._cleanup_connection()
                get_logger().info("定时任务执行前清理现有连接")
            
            # 重新检查连接状态
            if hasattr(self, '_check_connection_status'):
                self._check_connection_status()
                
            # 如果连接断开，尝试重新连接
            if not self.is_remote_connected:
                get_logger().warning("定时任务执行前检测到连接断开，尝试重新连接")
                # 触发立即重连检查
                if hasattr(self, 'reconnect_timer'):
                    self.reconnect_timer.start(100)  # 立即触发重连
                    # 等待重连完成
                    import time
                    time.sleep(2)
                    # 再次检查连接状态
            ...
            if self.is_remote_connected:
                get_logger().info("定时任务执行前连接状态: 已连接")
            else:
                get_logger().warning("定时任务执行前连接状态: 未连接")
                
        except Exception as e:
            get_logger().error(f"确保连接健康时发生异常: {e}")
    
    def _on_edit_schedule(self, task_id: str) -> None:
        """编辑定时任务。"""
        ...
        for i in range(dialog.type_combo.count()):
            if dialog.type_combo.itemData(i) == task.schedule_type:
                dialog.type_combo.setCurrentIndex(i)
                break
        
        dialog.time_input.setText(task.scheduled_time)
        dialog.command_input.setPlainText(task.command)
        
        if dialog.exec() == QDialog.DialogCode.Accepted:
            task_data = dialog.get_task_data()
            if task_data:
                # 更新任务
                if self.scheduler.update_task(
                    task_id,
                    name=task_data['name'],
                    command=task_data['command'],
                    schedule_type=task_data['schedule_type'],
                    ...
                    self._refresh_schedule_list()
                    get_logger().info(f"定时任务更新成功: {task_data['name']}")
    
    def _on_delete_schedule(self, task_id: str) -> None:
        """删除定时任务。"""
        task = self.scheduler.tasks.get(task_id)
        if not task:
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if self.scheduler.delete_task(task_id):
                self._refresh_schedule_list()
                get_logger().info(f"定时任务删除成功: {task.name}")
    ...
    def _on_session_selection_changed(self) -> None:
        ...
        self.current_session_id = f"session_{current_row + 1}"
    ...
    def _request_scroll_chat_to_bottom(self) -> None:
        """请求滚动到底：兼容布局异步重排，分多次尝试。"""
        self._scroll_to_bottom_pending = True
        QTimer.singleShot(0, self._scroll_chat_to_bottom_now)
        QTimer.singleShot(30, self._scroll_chat_to_bottom_now)
        QTimer.singleShot(120, self._scroll_chat_to_bottom_now)
        QTimer.singleShot(200, self._clear_scroll_to_bottom_pending)

    def _clear_scroll_to_bottom_pending(self) -> None:
        self._scroll_to_bottom_pending = False

    def _on_chat_scroll_range_changed(self, _min_value: int, _max_value: int) -> None:
        """内容高度变化时，若本轮有新消息则继续滚到底，避免停在中间。"""
    ...
    def _check_connection_status(self) -> None:
        ...
        try:
            ...
            # 使用连接状态检查而不是执行命令来检测连接
            is_connected = self._check_connection_status_only()
    ...
    def _check_connection_status_only(self) -> bool:
        """仅检查连接状态而不记录日志，避免重复连接记录。"""
        if not self.cfg.ssh_enabled or not self.cfg.ssh or not self.cfg.ssh.host:
            return False
        
        try:
            # 使用现有的连接状态检查，避免建立新连接
            if hasattr(self.orchestrator.executor, '_is_connection_valid'):
                # 如果连接器支持连接状态检查，使用它
                return self.orchestrator.executor._is_connection_valid()
            else:
                # 回退到简单的连接测试
                import socket
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(5)
                result = sock.connect_ex((self.cfg.ssh.host, self.cfg.ssh.port))
                sock.close()
                return result == 0
        except Exception:
            return False
...
def run_app() -> None:
    ...
    login_dialog = ServerLoginDialog(runtime_cfg)
    if login_dialog.exec() != QDialog.DialogCode.Accepted:
        get_logger().info("用户取消服务器登录，应用退出")
        return

    ssh_cfg = login_dialog.build_ssh_config()
    if ssh_cfg is None:
        get_logger().warning("登录配置校验失败，应用退出")
        return

    runtime_cfg.ssh_enabled = True
    runtime_cfg.ssh = ssh_cfg

    window = ChatWindow(cfg=runtime_cfg)
    window.show()
    sys.exit(app.exec())
