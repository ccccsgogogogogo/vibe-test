...
class IntentPlanner:
    ...
    def plan(self, user_text: str, profile: str) -> PlannedCommand:
        ...
        if any(k in text for k in ["create user", "add user", "新增用户", "添加用户", "创建用户"]):
            return PlannedCommand(
                intent="user_create",
        ...
        if any(k in text for k in ["version", "版本", "发行版", "系统版本"]):
            return PlannedCommand(intent="os_release", command="cat /etc/os-release; uname -a")

        if any(k in text for k in ["reboot", "restart server", "重启服务器", "重启主机"]):
            return PlannedCommand(intent="reboot", command="reboot", needs_confirmation=True)

        if any(k in text for k in ["update", "升级", "安装包", "apt", "yum", "dnf"]):
        ...
        if any(k in text for k in ["top", "资源", "性能", "性能概览"]):
