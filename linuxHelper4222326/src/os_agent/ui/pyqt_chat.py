...
class ScheduleDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        # 对话绑定提示
        session_info_label = QLabel("此定时任务将绑定到当前对话，只在当前对话中执行")
...
class ChatWindow(QMainWindow):
    ...
    def _create_schedule_item_widget(self, task: ScheduledTask) -> QWidget:
        ...
        # 状态指示器
        status_label = QLabel("✓ 启用" if task.enabled else "✗ 禁用")
    ...
    def _on_scheduled_task_execute(self, command: str, session_id: str) -> None:
        """定时任务执行回调。"""
        # 在主线程中执行命令
        QTimer.singleShot(0, lambda: self._execute_scheduled_command(command, session_id))
    
    def _execute_scheduled_command(self, command: str, session_id: str) -> None:
        """执行定时任务命令。"""
        # 检查是否在当前对话中执行
        if session_id != self.current_session_id:
            get_logger().info(f"定时任务属于对话 {session_id}，当前对话为 {self.current_session_id}，跳过执行")
            return
        
        get_logger().info(f"开始执行定时任务: {command}")
        ...
        try:
            ...
            # 3. 直接调用发送按钮的逻辑
            # 确保在UI线程中执行
            QTimer.singleShot(100, lambda: self._send_message_directly(command))
            
        except Exception as e:
            get_logger().error(f"定时任务执行失败: {e}")
            # 显示错误信息
            self._append("System", f"定时任务执行失败: {str(e)}")
